...
class PreviewCreator @Inject constructor(private val fileManager: FileManager) {
    ...
    fun createPreviewImage(
        ...
        val destinationFile =
            fileManager.createFile(fileOperationRequest, FileManager.SubFolder.VAULT)

        val fileOutputStream = FileOutputStream(destinationFile)

        val image = ThumbnailUtils.extractThumbnail(
            BitmapFactory.decodeFile(originalMediaFile.absolutePath),
            250,
            250,
    ...
    fun createPreviewVideo(
        ...
        val image = ThumbnailUtils.createVideoThumbnail(
            originalMediaFile.absolutePath,
            MediaStore.Images.Thumbnails.MINI_KIND
        )
