package com.matrix.applock.ui.browser
...
data class BrowserPageViewState(
    ...
    fun loadingVisibility(): Int =
        if (webPageState == WebPageState.LOADING) View.VISIBLE else View.INVISIBLE

    fun quickBrowsingVisibility(): Int =
        if (webPageState == WebPageState.NONE) View.VISIBLE else View.GONE
    ...
    fun complete(): BrowserPageViewState =
        copy(webPageState = WebPageState.LOADED, loadingProgress = 100)

    companion object {

        fun empty() = BrowserPageViewState("", WebPageState.NONE, 0, false)

        fun load(url: String, isBookmarked: Boolean) =
            BrowserPageViewState(url, WebPageState.REQUEST, 0, isBookmarked)
    }
